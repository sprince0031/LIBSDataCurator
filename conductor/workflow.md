...
8. **Commit Code Changes and Record Task Summary:**
   - Stage all code changes related to the task.
   - Obtain a clear, concise description for the completed task. This should include the task name, a summary of changes, a list of all created/modified files, and the core "why" for the change.
   - Commit changes with a message following the format `<type>(<scope>): <description>`. For example: `feat(ui): Create basic HTML structure for calculator`. Include the detailed summary in the commit body.

9. **Get and Record Task Commit SHA:**
   - **Step 9.1: Get Commit Hash:** Obtain the hash of the *just-completed commit* (`git log -1 --format="%H"`).
   - **Step 9.2: Update Plan:** Read `plan.md`, find the line for the completed task, update its status from `[~]` to `[x]`, and append the first 7 characters of the hash.
...
6.  **Create Checkpoint Commit:**
    ...
    -   Perform the commit with a clear and concise message (e.g., `conductor(checkpoint): Checkpoint end of Phase X`). Include a detailed verification report in the commit body.
